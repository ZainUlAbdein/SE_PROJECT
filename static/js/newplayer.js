...
// // pagination FUNCTION
// function LoadMusic(source_path) {
//   console.log(source_path);
//   music_title.querySelector("span").textContent = source_path;
...
function LoadMusicNew(source_path) {
  console.log(source_path);
  music_title.querySelector("span").textContent = source_path;
  audio.src = source_path;
  if (isNaN(audio.duration)) {
...
function audioVisualizer(audio) {
  ...
  function animate() {
    ...
    for (let i = 0; i < bufferLength; i++) {
      ...
      ctx.lineTo(x - barWidth / 2, canvas.height - dataArray[i - 1] - 100);
      ctx.stroke();
      ctx.closePath();
      ...
      // BAR
      ctx.fillStyle = `rgb(${color.red}, ${color.green}, ${color.blue})`;
      ctx.fillRect(x, canvas.height - barHeight - 75, barWidth, barHeight);
      x += barWidth;
    }
  }
  animate();
